...
class GA_Refiner:
    ...
    def refine_solution(self, products, args):
        ...
        _ = ga.evolve(
            ...
            crossover_rate=0,
            max_generations=(
                args["generations"] if "generations" in args else GENERATIONS
            ),
            **args
        )
                        ...
                        if prod.id == p:
                            ...
                            if (
                                len(
                                    args["domain_interval_tree"].overlap(
                                        prod.setup_beg[c], prod.setup_end[c]
                                    )
                                )
                                == 0
                            ):
                                ...
                                prod.setup_gap[c] = args["gap_at_day"][
                                    prod.setup_beg[c] // args["time_units_in_a_day"]
                                ]
                            ...
                            for l in range(len(elem["load_beg"])):
                                ...
                                if (
                                    len(
                                        args["domain_interval_tree"].overlap(
                                            prod.load_beg[c, l], prod.load_end[c, l]
                                        )
                                    )
                                    == 0
                                ):
                                    ...
                                    prod.load_gap[c, l] = args["gap_at_day"][
                                        prod.load_beg[c, l]
                                        // args["time_units_in_a_day"]
                                    ]
                                # Unload
                                ...
                                if (
                                    len(
                                        args["domain_interval_tree"].overlap(
                                            prod.unload_beg[c, l], prod.unload_end[c, l]
                                        )
                                    )
                                    == 0
                                ):
                                    ...
                                    prod.unload_gap[c, l] = args["gap_at_day"][
                                        prod.unload_beg[c, l]
                                        // args["time_units_in_a_day"]
                                    ]
                            # Cycle end
        ...
        for p, prod in products:
            for c in prod.setup_beg.keys():
                cycle = {
                    ...
                    "fixed": isinstance(
                        prod, RunningProduct
                    ),  # "fixed" basically flags if such cycle is a RunningProduct instance => it cannot be moved
                    # Dynamic information (will be updated during optimization)
                    "setup_beg": prod.setup_beg[c],
                    "setup_end": prod.setup_end[c],
                    "load_beg": [
                        prod.load_beg[c, l] for l in range(prod.num_levate[c])
                    ],
                    "load_end": [
                        prod.load_end[c, l] for l in range(prod.num_levate[c])
                    ],
                    "unload_beg": [
                        prod.unload_beg[c, l] for l in range(prod.num_levate[c])
                    ],
                    "unload_end": [
                        prod.unload_end[c, l] for l in range(prod.num_levate[c])
                    ],
                }
            ...
            for machine_queue in individual:
                for cycle in machine_queue:
                    ...
                    if cycle["unload_end"][-1] > args["makespan_limit"]:
                        penalty += (
                            cycle["unload_end"][-1] - args["makespan_limit"]
                        ) // 2
                ...
                individual_fitness += sum(
                    [cycle["unload_end"][-1] for cycle in machine_queue]
                )
            # Store fitness
            ...
            if TEMPERATURE_PROFILING:
                ...
                if random.random() < args["temperature"][args["curr_generation"]]:
                    ...
                    if mutation_choice == 0:
                        ...
                        individual = self._between_machines_swap(
                            individual, random, args
                        )
                    else:
                        pass
                else:
    ...
    def _compact_and_shift(self, individual, random, args):
        ...
        machine_candidates = [
            m for m in range(args["num_machines"]) if len(individual[m]) > 0
        ]
        if len(machine_candidates) == 0:
            return individual  # skip mutation if no machines are available
        ...
        # Adjust machine
        res = adjust_machines(
            random, individual, queues, anchors, adjustment_start, args
        )
    ...
    def _within_machine_swap(self, individual, random, args):
        ...
        machine_candidates = [
            ...
            if not (
                len(individual[m]) == 0
                or ((len(individual[m]) == 1) and individual[m][0]["fixed"])
            )
        ]
        if len(machine_candidates) == 0:
            return individual  # skip mutation if no machines are available
        ...
        candidate_cycles = list(
            range(len(machine_queue))
            if not machine_queue[0]["fixed"]
            else range(1, len(machine_queue))
        )  # discard fixed cycles from available choices
        if len(candidate_cycles) <= 1:
            return individual  # skip mutation if there aren't at least 2 cycles to swap
        ...
        anchor_point = (
            min(cycle_A["setup_beg"], cycle_B["setup_beg"])
            if start_point > 0
            else args["start_schedule"]
        )

        # Swap cycles
        machine_queue[cycle_A_pos], machine_queue[cycle_B_pos] = (
            machine_queue[cycle_B_pos],
            machine_queue[cycle_A_pos],
        )
        ...
        # Adjust machine
        res = adjust_machines(
            random, individual, machine_queues, anchors, adjustment_start, args
        )
        machine_queue = res[machine_idx]
    ...
    def _pop_and_push(self, individual, random, args):
        ...
        if len(source_machine_candidates) == 0:
            return individual  # skip mutation if no machines are available
        source_machine_idx = random.choice(source_machine_candidates)
        source_machine = individual[source_machine_idx]
        # discard fixed cycles from available choices
        source_candidate_pos = list(
            range(len(source_machine))
            if not source_machine[0]["fixed"]
            else range(1, len(source_machine))
        )
        ...
        if len(target_machine_candidates) == 0:
            return individual  # skip mutation if no target has no machines available
        ...
        target_candidates_pos = list(
            range(len(target_machine) + 1)
            if len(target_machine) == 0 or not target_machine[0]["fixed"]
            else range(1, len(target_machine) + 1)
        )
        target_pos = random.choice(target_candidates_pos)

        # set anchor point for source machine
        source_anchor = (
            source_cycle["setup_beg"] if source_pos > 0 else args["start_schedule"]
        )
        ...
        # Setup parameters for machine adjustment
        machine_queue = {
            source_machine_idx: source_machine,
            target_machine_idx: target_machine,
        }
        anchor = {source_machine_idx: source_anchor, target_machine_idx: target_anchor}
        adjustment_start = {
            source_machine_idx: source_pos,
            target_machine_idx: target_pos,
        }
        # Adjust machines
        res = adjust_machines(
            random, individual, machine_queue, anchor, adjustment_start, args
        )
    ...
    def _between_machines_swap(self, individual, random, args):
        ...
        if len(target_candidates_pos) == 0:
            return individual  # skip mutation if target machine has no cycles compatible with source machine
        target_pos = random.choice(target_candidates_pos)
        ...
        target_anchor = (
            target_cycle["setup_beg"] if target_pos > 0 else args["start_schedule"]
        )

        # Swap cycles
        source_machine[source_pos], target_machine[target_pos] = (
            target_machine[target_pos],
            source_machine[source_pos],
        )
